...
contract RedEnvelope {
    ...
    function view_balance() view public returns(uint){
        return address(this).balance;
    }
    function randNo() private returns (uint){
        randNonce++;
        return uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randNonce))) % 50;
    ...
    function withdraw() public{
        require(msg.sender == owner, "Not authenticated to delete");
        uint bal = address(this).balance;
        require(bal > 0, "Letter already empty");
        payable(msg.sender).transfer(bal);
        transactions.push(
            transaction_details (
                msg.sender,
